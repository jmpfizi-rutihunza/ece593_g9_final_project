	...
	mp_sequencer seqr_0, seqr_1, seqr_2, seqr_3;
		...
		mp_transaction tx;
			...
			//Acquire the lock before checking 
            		bucket_lock.get(1);
            		if (num_tx <= 0) begin 
				// If no transactions left, release lock and exit
               			bucket_lock.put(1);
				`uvm_info(id, "Pool is empty, core is retiring.", UVM_LOW)
                	break;
            	end
		// Decrement the counter IMMEDIATELY while holding the lock
            	num_tx--; 	
		// Release the lock so other cores can check the remaining count
            	bucket_lock.put(1);

            	
		// execute the transaction (outside the lock to allow parallelism)
        	tx = mp_transaction::type_id::create("tx");
        	if (!tx.randomize()) begin
            		`uvm_error(id, "Randomization failed")
        	end
        
        	// Start the sequence on the specific core sequencer passed to this task
        	start_item(tx, -1, s); 
        	finish_item(tx);
        
        	`uvm_info(id, $sformatf("Transaction completed. Remaining in pool: %0d", num_tx), UVM_HIGH)
        	end
    	endtask
